...
from gff3_to_sql import run_gff3_to_sql

# %%
ray.init()

# %%
sql_table_name = "arabidopsis"
data_dir = "."
# data_dir = "/data1/dhananjayraman"
file_format = "gff3"
file_name = f"{data_dir}/Arabidopsis_thaliana.TAIR10.60.gff3"
...
if file_format == "gtf":
    gr = pr.read_gtf(file_name)
else:
    gr = pr.read_gff3(file_name)
end_time = time.time()
if OUTPUT_MODE == "file":
...
# Call the function
start_time = time.time()
# lib.run_gtf_to_sql(sql_db_name.encode("utf-8"), sql_table_name.encode("utf-8"), file_name.encode("utf-8"), 8)
if file_format == "gtf":
    run_gtf_to_sql(sql_db_name, sql_table_name, file_name)
else:
    run_gff3_to_sql(sql_db_name, sql_table_name, file_name)
end_time = time.time()
if OUTPUT_MODE == "file":
...
if sql_table_name == "arabidopsis":
    time = get_ipython().run_line_magic("timeit",'-o (gr[gr.Feature == "exon"].df.groupby("gene_id",dropna=False).size().reset_index(name="exon_count"))')
else:
...
if sql_table_name == "arabidopsis":
    exon_counts_pr = (gr[gr.Feature == "exon"].df.groupby("gene_id",dropna=False).size().reset_index(name="exon_count"))
else:
...
if sql_table_name == "arabidopsis":
    # convert None to NaN in gene_id column of exon_counts_sql
    exon_counts_sql["gene_id"] = exon_counts_sql["gene_id"].fillna(np.nan)
...
# Using pyranges
if sql_table_name == "arabidopsis":
    time = get_ipython().run_line_magic("timeit",'-o exon_lengths_pr = (gr[gr.Feature == "exon"].df.assign(length=lambda df: df["End"] - df["Start"]).groupby("gene_id",dropna=False)["length"].sum().reset_index(name="total_exon_length"))')
else:
...
if sql_table_name == "arabidopsis":
    exon_lengths_pr = (gr[gr.Feature == "exon"].df.assign(length=lambda df: df["End"] - df["Start"]).groupby("gene_id", dropna=False)["length"].sum().reset_index(name="total_exon_length"))
else:
...
if sql_table_name == "arabidopsis":
    # convert None to NaN in gene_id column of exon_lengths_sql
    exon_lengths_sql["gene_id"] = exon_lengths_sql["gene_id"].fillna(np.nan)
...
# Using pyranges
if sql_table_name == "arabidopsis":
    time = get_ipython().run_line_magic("timeit",'-o transcript_counts_pr = gr[(gr.Feature == "mRNA") | (gr.Feature == "snoRNA") | (gr.Feature == "lnc_RNA") | (gr.Feature == "pseudogenic_transcript")].df["Chromosome"].value_counts().idxmax()')
else:
...
# using sql
if sql_table_name == "arabidopsis":
    time = get_ipython().run_line_magic("timeit",f'-o transcript_counts_sql = pd.read_sql_query("SELECT Chromosome, COUNT(*) as transcript_count FROM {sql_table_name} WHERE Feature = Feature IN (\'mRNA\', \'snoRNA\', \'lnc_RNA\', \'pseudogenic_transcript\') GROUP BY Chromosome ORDER BY transcript_count DESC LIMIT 1", conn)')
else:
...
if sql_table_name == "arabidopsis":
    transcript_counts_pr = gr[(gr.Feature == "mRNA") | (gr.Feature == "snoRNA") | (gr.Feature == "lnc_RNA") | (gr.Feature == "pseudogenic_transcript")].df["Chromosome"].value_counts().idxmax()
    transcript_counts_sql = pd.read_sql_query(f"SELECT Chromosome, COUNT(*) as transcript_count FROM {sql_table_name} WHERE Feature IN ('mRNA', 'snoRNA', 'lnc_RNA', 'pseudogenic_transcript') GROUP BY Chromosome ORDER BY transcript_count DESC LIMIT 1", conn)
else:
...
elif sql_table_name == "arabidopsis":
    other_genes_dict = {"Chromosome": ["1"], "Start": [300000], "End": [6000000], "Strand": ["+"]}
